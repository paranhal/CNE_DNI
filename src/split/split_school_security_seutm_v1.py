# -*- coding: utf-8 -*-
"""
학교별 보안장비 현황 엑셀 분리 스크립트 (seutm_v1)
- 스위치와 동일 구조·로직
- 원본: DJE_SEUTM_LIST.xlsx (대전, --source로 다른 파일 지정 가능)
- 출력: {학교명}_보안장비 장비 현황 상세.XLSX
- 로그: split_log_security_{지역}_{날짜}.csv (예: split_log_security_DNI_20260222.csv)
"""
import os
import csv
import re
...
SOURCE_BY_REGION = {
    "DNI": os.path.join(BASE_DIR, "DJE_SEUTM_LIST.xlsx"),
    "CNE": os.path.join(BASE_DIR, "CNE_SEUTM_LIST.xlsx"),
}
...
def get_today_log_path(region='DNI', suffix=None):
    base = f"split_log_security_{region}_{datetime.now().strftime('%Y%m%d')}"
    if suffix:
...
def get_processed_school_codes(region='DNI'):
    prefix = f"split_log_security_{region}_"
    processed = set()
...
def main(missed_only=False, source_file=None, only_schools=None, schools_file=None,
    ...
    sheet_name = None
    for candidate in ["SEC_대전전체", "보안_대전전체", "보안장비", "Security", "Sheet1"]:  # "SEC_충남전체","보안_충남전체" 주석
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='학교별 보안장비 현황 엑셀 분리 (seutm_v1)')
    ...
    parser.add_argument('--source', '-s', type=str, default=None,
                        help='원본 파일 (기본: DJE_SEUTM_LIST.xlsx)')
    parser.add_argument('--test', '-t', action='store_true',
                        help='테스트: OUTPUT 폴더에 생성. 없으면 실제 목표 저장 폴더에 저장')
    parser.add_argument('--DNI', '--dni', action='store_true', help='대전 (로그: split_log_security_DNI_YYYYMMDD.csv)')
    parser.add_argument('--CNE', '--cne', action='store_true', help='충남 (로그: split_log_security_CNE_YYYYMMDD.csv)')

# -*- coding: utf-8 -*-
"""
학교별 스위치 장비 현황 엑셀 분리 스크립트 (V1)
- 학교코드 기준 작업 (학교명은 지역별로 다를 수 있음)
- 원본 서식(행높이, 열넓이, 셀색상, 폰트, 맞춤) 유지
- A1 셀(제목) 데이터 및 병합 셀 복사
- OUTPUT/지역/학교명_학교코드/학교명_스위치 장비 현황 상세.XLSX
- 로그: split_log_switch_{지역}_{날짜}.csv (예: split_log_switch_DNI_20260222.csv)
- 서식·복사 규칙: AP와 동일
"""
import os
import csv
...
SOURCE_BY_REGION = {
    "DNI": os.path.join(BASE_DIR, "DJE_SWITCH_LIST.xlsx"),
    "CNE": os.path.join(BASE_DIR, "CNE_SWITCH_LIST.xlsx"),
}
...
DEVICE_NAME = "스위치"  # 파일명: {학교명}_{장비명} 장비 현황 상세.XLSX


def get_today_log_path(region='DNI', suffix=None):
    base = f"split_log_switch_{region}_{datetime.now().strftime('%Y%m%d')}"
    if suffix:
...
def get_processed_school_codes(region='DNI'):
    prefix = f"split_log_switch_{region}_"
    processed = set()
...
def main(missed_only=False, source_file=None, only_schools=None, schools_file=None,
    ...
    sheet_name = None
    for candidate in ["SW_대전전체", "스위치_대전전체", "스위치", "Switch", "Sheet1"]:  # "SW_충남전체","스위치_충남전체" 주석
    ...
    # 관리번호 열에서 학교코드 추출하여 매칭 (원본에서 학교코드 열 삭제됨)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='학교별 스위치 장비 현황 엑셀 분리 (V1)')
    ...
    parser.add_argument('--DNI', '--dni', action='store_true', help='대전 (로그: split_log_switch_DNI_YYYYMMDD.csv)')
    parser.add_argument('--CNE', '--cne', action='store_true', help='충남 (로그: split_log_switch_CNE_YYYYMMDD.csv)')
